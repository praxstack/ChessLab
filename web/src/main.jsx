...
import VisionTraining from './VisionTraining.jsx';
import EndgamePractice from './EndgamePractice.jsx';
import GameLibrary from './GameLibrary.jsx';
import PositionNotes from './PositionNotes.jsx';
import BoardDrawingTools from './BoardDrawingTools.jsx';
import {toggleMark} from '../../shared/board-marks.js';
import './styles.css';
...
function App() {
  ...
  const [endgameSelection,setEndgameSelection]=useState('king-in-front');
  ...
  const [importFormat,setImportFormat]=useState('pgn'),[markTool,setMarkTool]=useState('move'),[markColor,setMarkColor]=useState('Y'),[reviewPracticeSide,setReviewPracticeSide]=useState('both');
  ...
  const currentAnnotation=(study.annotations||[]).find(a=>a.branchId===(branchId||null)&&a.ply===visiblePly);
  const canDraw=page==='play'&&mode==='review'&&game&&(!branch||visiblePly>=branch.anchorPly);
  const drawingKey=[game?.id,branchId,visiblePly,historyKey,page,mode].join('|');
  const chess=useMemo(()=>{
    if(page==='puzzles'&&puzzle)return replay(puzzleMoves,puzzle.fen);
    if(page==='learn'&&lesson)return replay([],lesson.fen);
    return replay(history.slice(0,visiblePly),game?.initialFen,game?.variant);
  },[page,puzzle,puzzleMoves,lesson,historyKey,visiblePly,game?.initialFen,game?.variant]);
  const rows=useMemo(()=>moveRows(history,game?.initialFen,game?.variant),[historyKey,game?.initialFen,game?.variant]);
  ...
  useEffect(()=>{
    ...
    const controller=new AbortController(),timer=setTimeout(()=>api('/api/openings/recognize',{gameId:game.id,moves:history.slice(0,visiblePly),initialFen:game.initialFen,variant:game.variant},controller.signal).then(data=>setOpeningMatch(data.opening)).catch(()=>{}),140);
    return()=>{clearTimeout(timer);controller.abort();};
  },[page,mode,game?.id,historyKey,visiblePly,game?.initialFen,game?.variant]);
  ...
  async function botReply(current){
    if(current.result||current.source!=='bot'||replay(current.moves,current.initialFen,current.variant).turn()===current.color)return current;
  ...
  async function choosePractice(){
    ...
    setPracticeTarget({id:game.id,branchId,ply:visiblePly,fen:chess.fen(),variant:game.variant,title:game.title});setSetup(current=>({...current,color:chess.turn(),variant:game.variant||'standard',positionNumber:null,engineId:game.variant==='chess960'&&!catalog.engines.find(e=>e.id===current.engineId)?.chess960?'stockfish19':current.engineId}));setModal('practice');
  }
  ...
  async function authenticate(e){
    ...
    else if(action?.type==='import')await performImport(action.pgn,action.format);
    else if(action?.type==='position')await performPosition(action.payload);
    else if(action?.type==='opening')await performOpening(action);
    else if(action?.type==='endgame')await performEndgame(action);
  }
  ...
  function onSquare(square){if(!boardEnabled)return;const piece=chess.get(square);if(selected===square){setSelected(null);return;}if(selected&&(piece?.color!==chess.turn()||chess.moves({square:selected,verbose:true}).some(m=>m.to===square))){boardMove(selected,square);return;}if(piece?.color===chess.turn())setSelected(square);else setSelected(null);}
  ...
  async function performOpening({id,selectedPly,practice}){
    ...
    try{const data=await api(`/api/openings/${id}/study`,{}),next=data.game;adopt(next);setOpeningSelection(id);setPage('play');setMode('review');setPly(selectedPly);setModal(null);
      if(practice){const position=replay(next.moves.slice(0,selectedPly),next.initialFen,next.variant);setPracticeTarget({id:next.id,branchId:null,ply:selectedPly,fen:position.fen(),title:next.title});setSetup(current=>({...current,color:position.turn(),variant:next.variant||'standard',positionNumber:null}));setModal('practice');}
      await refreshGames();
    }catch(err){setError(err.message);}finally{endBusy();}
  }
  async function openEndgame(id,practice){const action={type:'endgame',id,practice};if(requireAccount(action))await performEndgame(action);}
  async function performEndgame({id,practice}){
    if(!await persistStudy())return;if(!startBusy())return;
    try{const {game:next}=await api(`/api/endgames/${id}/study`,{});adopt(next);setEndgameSelection(id);setPage('play');setMode('review');setPly(0);setModal(null);
      if(practice){const position=replay([],next.initialFen);setPracticeTarget({id:next.id,branchId:null,ply:0,fen:position.fen(),title:next.title});setSetup(current=>({...current,color:position.turn(),variant:'standard',positionNumber:null}));setModal('practice');}
      await refreshGames();
    }catch(err){setError(err.message);}finally{endBusy();}
  }
  async function openPositionEditor(){if(!await persistStudy())return;setPositionDraft({fen:page==='play'?chess.fen():null,variant:page==='play'?game?.variant:undefined,title:'Custom position'});setError('');setModal('position');}
  ...
  async function importPgn(e){e.preventDefault();if(requireAccount({type:'import',pgn,format:importFormat}))await performImport(pgn,importFormat);}
  async function performImport(text,format='pgn'){
    let payload;try{payload=format==='study'?JSON.parse(text):{pgn:text};}catch{setError('This study file is not valid JSON. Your current game is unchanged.');return;}
    if(!await persistStudy())return;if(!startBusy())return;
    try{const data=await api(format==='study'?'/api/import-study':'/api/import',payload);adopt(data.game,{restore:format==='study'});setMode('review');if(format!=='study')setPly(0);setPage('play');setModal(null);setPgn('');await refreshGames();setNotice(format==='study'?'Study restored as a separate saved game.':'PGN imported with comments and variations.');}catch(err){setError(err.message);}finally{endBusy();}
  }
  async function exportFile(format){
    if(!await persistStudy())return;if(!startBusy())return;
    try{const current=gameRef.current,response=await fetch(`/api/games/${current.id}/${format==='pgn'?'pgn':'study-file'}`);if(!response.ok)throw new Error('Could not export this study.');const text=await response.text(),url=URL.createObjectURL(new Blob([text],{type:format==='pgn'?'application/x-chess-pgn':'application/json'}));const link=document.createElement('a');link.href=url;link.download=`chesslab-${current.id.slice(0,8)}.${format==='pgn'?'pgn':'chesslab.json'}`;link.click();URL.revokeObjectURL(url);}catch(err){setError(err.message);}finally{endBusy();}
  }
  function updateAnnotation(value){
    const item={branchId:branchId||null,ply:visiblePly,comment:value.comment,nags:value.nags,...(value.marks!==undefined?{marks:value.marks}:{})};
    setStudy(old=>({...old,annotations:[...(old.annotations||[]).filter(a=>a.branchId!==item.branchId||a.ply!==item.ply),...(item.comment||item.nags.length||item.marks?.length?[item]:[])]}));setDirty(true);
  }
  function drawMark(mark){try{updateAnnotation({...currentAnnotation,comment:currentAnnotation?.comment||'',nags:currentAnnotation?.nags||[],marks:toggleMark(currentAnnotation?.marks||[],mark)});}catch(error){setError(error.message);}}
  function clearMarks(){updateAnnotation({...currentAnnotation,comment:currentAnnotation?.comment||'',nags:currentAnnotation?.nags||[],marks:[]});}
  ...
  function changePage(next){setNotice('');if(next==='learn')setLesson(null);exerciseSequence.current++;viewSequence.current++;setPage(next);setSelected(null);setFeedback(null);setAutoplay(false);setShowBefore(false);if(next==='library'&&user)refreshGames().catch(err=>setError(err.message));}
  function setPreference(key,value){setSettings(current=>({...current,[key]:key==='coordinates'?(value===true||value==='true'):value}));}
  const lastSquares=last?[last.from,last.castle?.kingTo||last.to]:null;
  ...
  const pageTitle=page==='endgames'?'Endgames':page==='vision'?'Vision':page==='training'?'Puzzles':page==='openings'?'Openings':page==='learn'?'Learn the essentials':page==='puzzles'?'Find the next move':page==='library'?'Your games & studies':mode==='review'?'Game review':'Play bots';
  ...
  return <div className="app-shell" data-page={page} data-board-theme={settings.boardTheme}>
    <aside className="navigation"><a className="brand" href="#" aria-label="ChessLab home" onClick={e=>{e.preventDefault();changePage('play');}}><span className="brand-mark"><Piece type="n" color="w"/></span><span>Chess<span className="brand-light">Lab</span></span></a>
      <p className="nav-section">Play & practice</p><nav aria-label="Main navigation">{[['play','Play bots','play'],['training','Puzzles','puzzle'],['learn','Learn','learn'],['openings','Openings','openings'],['endgames','Endgames','endgames'],['vision','Vision','vision'],['library','My games','library']].map(([id,label,icon])=><button key={id} aria-label={label} className={`nav-item ${page===id||(id==='training'&&page==='puzzles')?'active':''}`} onClick={()=>changePage(id)}><img className="nav-asset" src={`/icons/${{play:'play-white',puzzle:'puzzle-piece',learn:'lessons',openings:'binoculars',vision:'training',endgames:'lessons',library:'training'}[icon]}.svg`} alt=""/><span>{label}</span>{id==='play'&&<span className="nav-dot"/>}</button>)}</nav>
    ...
    <main className="main"><header className="page-header"><div><p className="eyebrow">{page==='play'?'YOUR DAILY CHESS':page==='library'?'YOUR COLLECTION':'PRACTICE'}</p><h1>{pageTitle}</h1></div><div className="header-actions"><button className="icon-button mobile-account" aria-label={user?'Your account':'Sign in'} onClick={()=>{pendingAction.current=null;setModal(user?'account':'auth');}}><Icon name="user"/></button><span className={`engine-status ${status?.engine?.available?'ready':''}`}><span className="status-dot"/>{status?.engine?.available?'Coach online':status?'Engine unavailable':'Connecting'}</span><button className="icon-button" title="Settings" aria-label="Open settings" onClick={()=>setModal('settings')}><Icon name="settings"/></button></div></header>
      ...
      {page==='endgames'?<EndgamePractice engines={catalog.engines} key={user?.id||'guest'} request={api} user={user} settings={settings} busy={busy||saving} initialId={endgameSelection} onOpen={openEndgame} onGame={openGame}/>:page==='vision'?<VisionTraining request={api} user={user} settings={settings} onSignIn={()=>requireAccount({type:'vision'})}/>:page==='learn'&&!lesson?<LearningPath request={api} user={user} settings={settings} starterLessons={learn.lessons} completed={progress.lessons} onSignIn={()=>requireAccount({type:'course'})} onStarter={selectLesson} onProgress={lessons=>setProgress(p=>({...p,lessons}))}/>:page==='training'?<PuzzleTraining request={api} user={user} settings={settings} onSignIn={()=>requireAccount({type:'training'})} onStarter={()=>changePage('puzzles')} onStudy={openTrainingStudy}/>:page==='openings'?<OpeningLibrary request={api} settings={settings} initialId={openingSelection} user={user} onStudy={async text=>{if(requireAccount({type:'import',pgn:text,format:'pgn'}))await performImport(text);}} onOpen={openOpening} busy={busy}/>:page==='library'?<GameLibrary key={user?.id||'guest'} api={api} user={user} games={games} busy={busy||saving} onOpen={openGame} onNew={()=>setModal('new')} onImport={()=>setModal('import')} onPosition={openPositionEditor} onAuth={()=>{pendingAction.current=null;setModal('auth');}} Modal={Modal}/>:<div className="workspace"><section className="board-column">
        ...
        {canDraw&&<BoardDrawingTools tool={markTool} color={markColor} marks={currentAnnotation?.marks||[]} disabled={busy||saving} onTool={value=>{setMarkTool(value);setSelected(null);setMarkColor(value==='square'?'R':'Y');}} onColor={setMarkColor} onClear={clearMarks}/>}
        {page==='play'&&game?.variant==='chess960'&&<p className="variant-note">Chess960{Number.isInteger(game.positionNumber)?` · Position ${game.positionNumber}`:''} · To castle, move your king onto its rook, or type O-O / O-O-O.</p>}{page==='play'&&game&&<div className="board-actions"><button className="text-button" onClick={()=>setModal('new')}><Icon name="plus"/>New game</button>{game.source==='bot'&&<button className="text-button" disabled={busy} onClick={rematch}>{game.practice?'Restart position':'Rematch'}</button>}<button className="text-button" onClick={()=>setModal('import')}><Icon name="upload"/>Import</button><button className="text-button" disabled={busy||saving} onClick={openPositionEditor}>Set up position</button><button className="text-button" disabled={busy||saving} onClick={()=>exportFile('pgn')}><Icon name="download"/>Export PGN</button><button className="text-button" disabled={busy||saving} onClick={()=>exportFile('study')}>Export study</button>{game.source==='bot'&&!game.result&&<button className="text-button resign" disabled={busy} onClick={()=>setModal('resign')}><Icon name="flag"/>Resign</button>}</div>}
      </section>
      <section className="coach-panel">
        {page==='play'?<><div className="panel-tabs"><button className={mode==='play'?'active':''} onClick={()=>game?resumeGame():setMode('play')}>Play bots</button><button className={mode==='review'&&analysisScope==='review'?'active':''} disabled={!game} onClick={()=>{setMode('review');setAnalysisScope('review');setShowBefore(false);}}>Review</button><button className={mode==='review'&&analysisScope==='analysis'?'active':''} disabled={!game} onClick={()=>{setMode('review');setAnalysisScope('analysis');setShowBefore(false);}}>Analysis</button></div>
          ...
          <>{mode==='review'&&openingMatch&&<div className="opening-recognition"><span className="eco-code">{openingMatch.eco}</span><div><strong>{openingMatch.name}</strong><small>{openingMatch.matchType==='line'?'Played line':'Position match'} · at half-move {openingMatch.ply}</small></div><button className="text-button" onClick={async()=>{if(!await persistStudy())return;setOpeningSelection(openingMatch.id);setPage('openings');}}>Explore opening<Icon name="arrow"/></button></div>}{game.practice&&<div className="practice-origin"><div><span>POSITION PRACTICE</span><strong>{game.practice.sourceTitle}</strong><small>{game.practice.endgameId?'Original endgame position':`From ${game.practice.sourceBranchId?'a saved variation':'the original game'} · ${game.practice.startPly} prior plies`}</small></div><button className="text-button" disabled={busy||saving} onClick={returnToSource}>Return to source study<Icon name="arrow"/></button></div>}{mode==='review'&&analysisScope==='review'&&<GameReview onPractice={async side=>{if(!await persistStudy())return;setReviewPracticeSide(side);setModal('reviewpractice');}} key={game.id} game={game} settings={settings} selectedPly={branchId?-1:ply} onSelect={value=>{setBranchId(null);setAnalysisScope('review');jump(value);}} onReport={setGameReport} request={api}/>}<div className={`coach-message ${analyzing?'is-loading':''}`}><div className="coach-message-heading">{settings.coachAvatar&&<span className="mini-knight"><Piece type="n" color="w"/></span>}<strong>{branchId?'Variation lab':mode==='play'?'At your side':selectedMove?`${selectedMove.number}${selectedMove.color==='b'?'…':'.'} ${selectedMove.san}`:'Starting position'}</strong>{settings.classification&&analysis?.played&&<span className={`classification ${analysis.played.classification}`}>{analysis.played.classification}</span>}</div>
          ...
          {mode==='review'&&(!branch||visiblePly>=branch.anchorPly)&&<PositionNotes annotation={currentAnnotation} position={`${branchId?`Variation ${study.branches.findIndex(b=>b.id===branchId)+1}`:'Original game'} · ply ${visiblePly}${showBefore?' · before selected move':''}`} ply={visiblePly} onChange={updateAnnotation} onSave={persistStudy} dirty={dirty} disabled={busy||saving}/>}
          ...
          {study.branches.length>0&&<div className="saved-branches"><div className="section-label"><span>YOUR VARIATIONS · {study.branches.length}</span><button className="text-button" disabled={!dirty||busy||saving} onClick={persistStudy}>{saving?'Saving…':dirty?'Save study':'Saved'}</button></div>{study.branches.map((item,index)=><button className={`branch-item ${branchId===item.id?'active':''}`} key={item.id} style={{paddingLeft:12+Math.min(4,branchDepth(item,study.branches))*14}} onClick={()=>selectBranch(item)}><Icon name="branch"/><span>Line {index+1}<small>from ply {item.anchorPly} · {item.moves.length-item.anchorPly} {item.moves.length-item.anchorPly===1?'move':'moves'}</small></span>{item.question&&<span className="note-mark">NOTE</span>}</button>)}</div>}
    ...
    {modal==='position'&&<Modal title="Set up position" onClose={()=>setModal(null)} wide><PositionEditor initialFen={positionDraft?.fen} initialTitle={positionDraft?.title} initialVariant={positionDraft?.variant} settings={settings} busy={busy} error={error} onSave={savePosition}/></Modal>}
    {modal==='new'&&<Modal title="New bot game" onClose={()=>setModal(null)} wide><BotSetup {...setupProps} compact/></Modal>}
    {modal==='reviewpractice'&&game&&<Modal title="Practice your key moves" onClose={()=>setModal(null)} wide><ReviewPractice game={game} settings={settings} side={reviewPracticeSide} request={api}/></Modal>}
    {modal==='practice'&&practiceTarget&&<Modal title="Practice this position" onClose={()=>{setModal(null);setPracticeTarget(null);}} wide><div className="practice-preview"><div className="practice-preview-board" inert><Board chess={replay([],practiceTarget.fen,practiceTarget.variant)} orientation={setup.color==='b'?'b':'w'} enabled={false} onSquare={()=>{}} onMove={()=>{}} settings={{...settings,coordinates:false,animation:0}}/></div><div><span>{practiceTarget.branchId?'SAVED VARIATION':'GAME POSITION'}</span><h3>{replay([],practiceTarget.fen,practiceTarget.variant).turn()==='w'?'White':'Black'} to move</h3><p>Choose your opponent and side. Practice creates a separate game and keeps the full history of this position.</p></div></div>{error&&<p className="form-error" role="alert">{error}</p>}<BotSetup {...setupProps} compact variantLocked onStart={()=>createPractice()} startLabel="Start position practice"/><p className="fine-print">Practice games do not award roster crowns. You can return to the source study at any time.</p></Modal>}
    ...
    {modal==='import'&&<Modal title="Import a game" onClose={()=>setModal(null)} wide><p className="modal-intro">PGN brings in moves, comments and variations. A ChessLab study file restores the exact tree, questions and return point.</p><form onSubmit={importPgn}><label className="study-import-format">File format<select aria-label="Import format" value={importFormat} onChange={e=>setImportFormat(e.target.value)}><option value="pgn">Annotated PGN</option><option value="study">ChessLab study file</option></select></label><label className="file-import">Choose a .pgn or .chesslab.json file<input type="file" accept=".pgn,.json,text/plain,application/json" onChange={async e=>{const file=e.target.files?.[0];if(file){if(file.size>900000){setError('Choose a file smaller than 900 KB.');return;}try{const text=await file.text();setImportFormat(file.name.toLowerCase().endsWith('.json')?'study':'pgn');setPgn(text);}catch{setError('The file could not be read. Your draft is unchanged.');}}}}/></label><label className="sr-only" htmlFor="pgn-input">{importFormat==='pgn'?'PGN text':'Study JSON'}</label><textarea id="pgn-input" className="pgn-input" value={pgn} onChange={e=>setPgn(e.target.value)} maxLength={900000} placeholder={importFormat==='pgn'?'[Event "My game"]\n\n1. e4 {My idea} e5 (1... c5) *':'Paste a ChessLab study file here'} required/>{error&&<p className="form-error" role="alert">{error}</p>}<button className="button primary full-width" disabled={busy||!pgn.trim()}>{busy?'Importing…':user?'Import & review':'Sign in to import'}<Icon name="arrow"/></button><p className="tiny-note">Imports create a separate saved game. Study files preserve exact metadata; PGN normalizes continued and empty variations and comment braces.</p></form></Modal>}
