...
function App() {
  ...
  function adopt(next,{restore=false}={}) {
    viewSequence.current++;gameRef.current=next;studyBaselineRevision.current=next.studyRevision??0;setGame(next);setStudy(next.study||emptyStudy());setDirty(false);setSelected(null);setAnalysis(null);setHelp(null);setHint(null);setHelpError('');setShowBefore(false);setAutoplay(false);
  ...
  },[page,mode,game?.id,historyKey,ply,settings.movetime,settings.lines,settings.threads,settings.analysisEngine,settings.reviewStrength,analysisScope,analysisTick]);
  ...
  async function rematch(){await createGame(rematchOptions(game,setup));}
  ...
  const liveLabel=game?.result ? (game.result==='1/2-1/2'?'Draw':game.result===(game.color==='w'?'1-0':'0-1')?'You won':'Game finished') : chess.isCheck()?'King in check':busy?'Waiting for a reply…':engineTurn?`${game?.botName||'Engine'} to move`:`${chess.turn()==='w'?'White':'Black'} to move`;
  const pageTitle=page==='learn'?'Learn the essentials':page==='puzzles'?'Find the next move':page==='library'?'Your games & studies':mode==='review'?'Game review':'Play bots';
  ...
  return <div className="app-shell" data-page={page} data-board-theme={settings.boardTheme}>
    ...
    <main className="main"><header className="page-header"><div><p className="eyebrow">CHESSLAB / {page==='play'?'THE BOARD':page==='library'?'YOUR COLLECTION':'PRACTICE'}</p><h1>{pageTitle}<span className="title-dot">.</span></h1></div><div className="header-actions"><button className="icon-button mobile-account" aria-label={user?'Your account':'Sign in'} onClick={()=>{pendingAction.current=null;setModal(user?'account':'auth');}}><Icon name="user"/></button><span className={`engine-status ${status?.engine?.available?'ready':''}`}><span className="status-dot"/>{status?.engine?.available?'Coach online':status?'Engine unavailable':'Connecting'}</span><button className="icon-button" title="Settings" aria-label="Open settings" onClick={()=>setModal('settings')}><Icon name="settings"/></button></div></header>
      ...
      {page==='library'?<section className="library-page"><div className="collection-heading"><div><span className="eyebrow">PICK UP WHERE YOU LEFT OFF</span><h2>Every game has another lesson.</h2></div><button className="button primary" onClick={()=>setModal('new')}><Icon name="plus"/>New game</button></div>{!user?<div className="empty-state"><Icon name="library"/><h3>A place for your games.</h3><p>Sign in to save games, variations and your learning progress on this server.</p><button className="button primary" onClick={()=>{pendingAction.current=null;setModal('auth');}}>Create an account</button></div>:games.length===0?<div className="empty-state"><h3>Your first game is waiting.</h3><p>Start with a coach game or import a PGN to look closer.</p><button className="button secondary" onClick={()=>setModal('import')}>Import PGN</button></div>:<div className="games-grid">{games.map(item=><button className="game-card" key={item.id} onClick={()=>openGame(item.id)} disabled={busy}><span className="game-card-icon"><Piece type={item.source==='import'?'b':'n'} color={item.color}/></span><span className="game-card-copy"><small>{item.source==='import'?'IMPORTED GAME':`${item.botName||item.engineId||'COACH'} · ${item.rating||`LEVEL ${item.level}`}`}</small><strong>{item.title}</strong><span>{Math.ceil(item.moves.length/2)} moves · {item.result||'In progress'}{item.study?.branches?.length?` · ${item.study.branches.length} variations`:''}</span><time>{new Date(item.updatedAt).toLocaleDateString(undefined,{month:'short',day:'numeric'})}</time></span><Icon name="chevron"/></button>)}</div>}<button className="text-button" onClick={()=>setModal('import')}><Icon name="upload"/>Import a PGN</button></section>:
      <div className="workspace"><section className="board-column">
        <div className="player-row"><span className="player-avatar coach-avatar">{page==='play'?<BotAvatar bot={selectedBot}/>:<Piece type={page==='learn'?'b':'q'} color="b"/>}</span><div className="player-copy"><strong>{page==='learn'?'Learn by looking':page==='puzzles'?'Calculation practice':game?.source==='import'?'Imported game':game?.botName||selectedBot?.name||selectedEngine?.name||'Choose your opponent'}</strong><span>{page==='play'?`${game?.currentRating||game?.rating||setup.rating} · ${selectedEngine?.name||game?.engineId||'Server engine'}${game?.adaptive?' · Adaptive':''}`:page==='learn'?'Original introductory lessons':puzzle?.theme||'Original introductory puzzles'}</span></div>{page==='play'&&game?.source==='bot'?<span className={`game-clock ${engineTurn?'active':''}`} aria-label="Opponent clock">{clockText(game,game.color==='w'?'b':'w',clockNow)}</span>:<span className="player-tag">{page==='play'?'BOT':page==='learn'?'LEARN':'SOLVE'}</span>}<button className="icon-button board-settings" aria-label="Board settings" onClick={()=>setModal('settings')}><Icon name="settings"/></button></div>
        <div className="board-wrap"><div className={`evaluation-rail ${page==='play'&&mode==='play'&&!game?.assistance?.evaluation?'evaluation-hidden':''}`} title="Evaluation from White’s perspective"><div className="evaluation-white" style={{height:displayedScore?`${Math.max(5,Math.min(95,50+(displayedScore.type==='mate'?Math.sign(displayedScore.value)*45:displayedScore.value/20)))}%`:'50%'}}/><span>{scoreText(displayedScore)}</span></div><Board chess={chess} orientation={orientation} selected={selected} onSquare={onSquare} onMove={boardMove} enabled={boardEnabled} settings={settings} arrows={arrowMoves} lastMove={lastSquares} classification={settings.classification&&mode==='review'&&!showBefore?analysis?.played?.classification:null} label={`${page==='puzzles'?'Puzzle':'Game'} board. ${chess.turn()==='w'?'White':'Black'} to move`}/></div>
        <div className="player-row own-player"><span className="player-avatar own-avatar">{user?.username?.slice(0,1).toUpperCase()||<Icon name="user"/>}</span><div className="player-copy"><strong>{user?.username||'You'}<span className="color-label">{orientation==='w'?'White side':'Black side'}</span></strong><span>{page==='play'?(branchId?'Exploring a variation':showBefore?'Position before the selected move':game?liveLabel:'Ready when you are'):page==='learn'?'Read the position, then choose an answer':puzzleComplete?'Puzzle complete':puzzle?`${puzzle.side==='w'?'White':'Black'} to move`:'Choose a puzzle to begin'}</span></div>{page==='play'&&game?.source==='bot'&&<span className={`game-clock ${isLive&&!engineTurn?'active':''}`} aria-label="Your clock">{clockText(game,game.color,clockNow)}</span>}<button className="icon-button" title="Flip board" aria-label="Flip board" onClick={()=>setPreference('orientation',orientation==='w'?'b':'w')}><Icon name="flip"/></button></div>
        <div className="board-bottom"><form className="move-entry" onSubmit={e=>{e.preventDefault();submitMove(typed);}}><label className="sr-only" htmlFor="move-entry">Enter a move in SAN or UCI</label><span className="move-prompt">›</span><input id="move-entry" autoComplete="off" value={typed} onChange={e=>setTyped(e.target.value)} placeholder={boardEnabled?'Enter move · e4 or e2e4':'Move entry · select a playable position'} disabled={!boardEnabled}/><button aria-label="Play entered move" disabled={!boardEnabled||!typed.trim()}><Icon name="arrow"/></button></form><span className="keyboard-hint">{boardEnabled?'Click, drag, or type a move':'← → to review moves'}</span></div>
        {page==='play'&&game&&<div className="board-actions"><button className="text-button" onClick={()=>setModal('new')}><Icon name="plus"/>New game</button>{game.source==='bot'&&<button className="text-button" disabled={busy} onClick={rematch}>Rematch</button>}<button className="text-button" onClick={()=>setModal('import')}><Icon name="upload"/>Import</button><button className="text-button" onClick={exportPgn}><Icon name="download"/>Export PGN</button>{game.source==='bot'&&!game.result&&<button className="text-button resign" disabled={busy} onClick={()=>setModal('resign')}><Icon name="flag"/>Resign</button>}</div>}
      </section>
      <section className="coach-panel">
        {page==='play'?<><div className="panel-tabs"><button className={mode==='play'?'active':''} onClick={()=>game?resumeGame():setMode('play')}>Play bots</button><button className={mode==='review'&&analysisScope==='review'?'active':''} disabled={!game} onClick={()=>{setMode('review');setAnalysisScope('review');setShowBefore(false);}}>Review</button><button className={mode==='review'&&analysisScope==='analysis'?'active':''} disabled={!game} onClick={()=>{setMode('review');setAnalysisScope('analysis');setShowBefore(false);}}>Analysis</button></div>
          ...
          <><div className={`coach-message ${analyzing?'is-loading':''}`}><div className="coach-message-heading">{settings.coachAvatar&&<span className="mini-knight"><Piece type="n" color="w"/></span>}<strong>{branchId?'Variation lab':mode==='play'?'At your side':selectedMove?`${selectedMove.number}${selectedMove.color==='b'?'…':'.'} ${selectedMove.san}`:'Starting position'}</strong>{settings.classification&&analysis?.played&&<span className={`classification ${analysis.played.classification}`}>{analysis.played.classification}</span>}</div>
