...
function App() {
  ...
  return <div className="app-shell" data-page={page} data-board-theme={settings.boardTheme}>
    <aside className="navigation"><a className="brand" href="#" aria-label="ChessLab home" onClick={e=>{e.preventDefault();changePage('play');}}><span className="brand-mark"><Piece type="n" color="w"/></span><span>Chess<span className="brand-light">Lab</span></span></a>
      <p className="nav-section">Play & practice</p><nav aria-label="Main navigation">{[['play','Play bots','play'],['puzzles','Puzzles','puzzle'],['learn','Learn','learn'],['library','My games','library']].map(([id,label,icon])=><button key={id} aria-label={label} className={`nav-item ${page===id?'active':''}`} onClick={()=>changePage(id)}><img className="nav-asset" src={`/icons/${{play:'play-white',puzzle:'puzzle-piece',learn:'lessons',library:'training'}[icon]}.svg`} alt=""/><span>{label}</span>{id==='play'&&<span className="nav-dot"/>}</button>)}</nav>
