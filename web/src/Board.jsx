...
export default function Board({chess, orientation='w', selected, onSquare, onMove, enabled, settings, arrows=[], lastMove, classification, label, editing=false}) {
  ...
  return <div className="board" role="group" aria-label={label || 'Chess board'}>
    {[...ranks].flatMap((rank,row)=>[...files].map((file,col)=>{
      ...
      return <button type="button" key={square} className={`square ${dark?'dark':'light'} ${selected===square?'selected':''} ${settings.lastMove && lastMove?.includes(square)?'last-move':''} ${check?'in-check':''}`} aria-label={`${square}${piece?`, ${piece.color==='w'?'white':'black'} ${names[piece.type]}`:', empty'}${targets.has(square)?', legal destination':''}${check?', in check':''}`} onClick={()=>onSquare(square)} draggable={enabled && !!piece && (editing || piece.color===chess.turn())} onDragStart={event=>{event.dataTransfer.setData('text/plain',square);event.dataTransfer.effectAllowed='move';if(!editing)onSquare(square);}} onDragOver={event=>enabled&&event.preventDefault()} onDrop={event=>{event.preventDefault();const from=event.dataTransfer.getData('text/plain');if(enabled&&(/^[a-h][1-8]$/.test(from)||(editing&&/^[wb][pnbrqk]$/.test(from))))onMove(from,square);}}>
