...
(async () => {
  ...
  try {
    ...
    const receipt = { status: 'pass', checkedAt: new Date().toISOString(), engine: review.engine, checks: ['register and session cookie', 'typed e4 and real legal bot reply', 'engine review shown', 'saved game restored after reload', 'wrong/right lesson answers', 'wrong/right puzzle moves', 'progress restored after reload', 'bot search and visible identity', 'hint with engine evidence', 'nested study preserves original moves', 'board preference persists', 'mobile layout and account/settings access', 'no page errors or unexpected HTTP/model requests'], limitations: ['Uses isolated temporary SQLite, not the user database', 'Does not establish deployment or cross-browser support'] };
