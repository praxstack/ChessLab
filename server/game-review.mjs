...
export const positionKey=game=>hash(game.variant==='chess960'?[2,game.variant,game.initialFen,game.moves]:[1,game.initialFen,game.moves]);
...
export function appendReview(report,game,analysis){
 const index=report.entries.length,board=replay(game.moves.slice(0,index),game.initialFen,game.variant),color=board.turn();
 ...
 const after=replay(game.moves.slice(0,index+1),game.initialFen,game.variant);
