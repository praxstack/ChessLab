...
async function fixture(options={}) {
 ...
 let state=createApp({databasePath:join(temp,'db.sqlite'),engineApi});
...
test('nested studies reject altered history, missing parents, loops, illegal moves and survive bot races',async()=>{
 ...
 try{
  ...
  release({bestmove:'e7e5'});const bot=await botPromise;assert.deepEqual(bot.body.game.study,study);
